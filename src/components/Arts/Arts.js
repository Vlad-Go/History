...
export class Arts extends AppComponent {
  constructor(store) {
    super({
      store,
      listeners: ['click'],
  ...
  async setData() {
    this.data = await getData('./data/arts.json');
  }
  ...
  async onClick(e) {
    ...
    if (elementRole === 'back') {
      ...
      await openModal(TestsSetUp, this.store, 'arts');
