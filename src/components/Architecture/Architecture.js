...
export class Architecture extends AppComponent {
  constructor(store) {
    super({
      store,
      listeners: ['click'],
    ...
    this.data;
    this.state = this.getState('architecture');
  ...
  async setData() {
    this.data = await getData('./data/architecture.json');
  }
  ...
  async onClick(e) {
    ...
    if (elementRole === 'back') {
      ...
      await openModal(TestsSetUp, this.store, 'architecture');
