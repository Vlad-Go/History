...
export class Personalities extends AppComponent {
  constructor(store) {
    super({
      store,
      listeners: ['click'],
    ...
    this.data;
    this.state = this.getState('personalities');
  ...
  async setData() {
    this.data = await getData('./data/personalities.json');
  }
  ...
  async onClick(e) {
    ...
    if (elementRole === 'back') {
      ...
      await openModal(TestsSetUp, this.store, 'personalities');
  ...
  onDone(state) {
    this.state = state.personalities;
    this.currentTopic = this.state.currentTopicIndex;
    this.completedTopics = this.state.completedTopics;
